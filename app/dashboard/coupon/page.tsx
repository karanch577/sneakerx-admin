...
function Coupon() {
  ...
  >({
    ...
    queryFn: async () => {
      const res: AxiosResponse<GetCouponsRes> = await axios.get(
        `${baseUrl}/coupon/all?page=${page}&limit=10`,
        {
  ...
  const { mutate: handleDelete } = useMutation({
    mutationFn: async (data: Coupon) => {
      const res = await axios.delete(`${baseUrl}/coupon/${data._id}`, {
        withCredentials: true,
      });
      return res.data;
    },
  ...
  const handleCouponDelete = (data: Coupon) => {
    handleDelete(data);
  };
        ...
        return (
          <div className="flex gap-3">
            <div className="cursor-pointer">
              <EditCoupon data={data} refetch={refetch} />
            </div>
            <div
              className="cursor-pointer"
              onClick={() => handleCouponDelete(data)}
            >
              <Trash2 />
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent>
          ...
          {data && data.coupons.length > 0 && (
            <div>
              <TableList data={data.coupons} header={couponTableHeader} />
              <div className="mt-5">
                <PaginationComponent
